...
- [x] **F03 — Office customization:** delivered 13 milestone-unlocked choices across room themes, desk finishes, plants, and cats. Temporary live preview (including locked items), explicit free apply, reversible selections, automatic room progression option, and save/backup persistence. Reuses reviewed art with fixed desk and character anchors. Entry: 장비 → 사무실 꾸미기 or 업무 업그레이드. Implemented next at user request to work on something else after coworker work.
  - [x] Character appearance follow-up: coarser 16-frame worker, 4 hair colors × 4 shirt colors × 3 skin tones (48 combinations) via a runtime palette shader; free preview/apply and saved combinations. Live iOS Simulator review passed on 2026-09-26.
- [x] **Quality of life (outside roadmap, user-requested):** hold-to-repeat purchases for 업무 업그레이드 and 능력 훈련 (serial saves, stops at cost/level cap, on release, or when backgrounded), plus a navy backdrop UI refresh with rounded panels, buttons and sheets.
...
| 2026-09-26 | F03 character appearance, hold-to-repeat upgrades, navy UI refresh | `python3 tools/verify.py`: clean analysis, 55 tests (adds hold upgrade/training cost-cap-save flows and repeat-button lifecycle). All 16 changed macOS references compared individually against HEAD: 12 scene/decorated/boss references differ only within the worker bounds (new coarser sprite; chair and shoes now come from the new atlas as neutral gray); 4 full-screen references also reflect the navy backdrop/rounded-panel restyle. Desk, props, cat and boss unchanged. Live iPhone 17 Pro (iOS Simulator): existing level-14 save loaded; recorded typing, deadline, report, boss arrival/scolding/departure and transitions, with the custom palette also animating in the preview; shader recolors only its channel on iOS; hair/shirt/skin applied and persisted across force-quit; holding 전문성 training bought 15 levels and stopped on release. | Observed once during testing: `SqliteException(14)` on autosave after repeated reinstalls; not reproduced after a clean relaunch, and save code is unchanged. The save-error view dumps the raw save payload and overflows (pre-existing). Non-macOS goldens still need platform-specific review. F02 follow-up/F04 next. |
