...
extension _Screens on _GameScreenState {
  Widget _office() => Column(
    ...
    children: [
      ...
      PixelPanel(
        ...
        child: Row(
          children: [
            const PixelIcon('coin', size: 34, color: Color(0xffa98538)),
                ...
                children: [
                  ...
                  Semantics(
                    ...
                    child: Tooltip(
                      ...
                      child: Text(
                        won(s.cash),
                        style: const TextStyle(fontSize: 30, color: ink),
                      ),
                    ),
                  ),
                ],
        ...
        child: Column(
          children: [
                ...
                if (s.equipped.isNotEmpty)
                  Positioned(
                    ...
                    child: Wrap(
                      spacing: 4,
                      children: [
                        for (final slot in s.equipped.keys)
                          Container(
                            padding: const EdgeInsets.all(4),
                            color: paper,
                            child: PixelIcon(slot, size: 14, color: green),
                          ),
                      ],
                    ),
                  ),
            ...
            Padding(
              ...
              child: Row(
                children: [
                  const Text('●', style: TextStyle(fontSize: 10, color: green)),
      ...
      const SizedBox(height: 12),
      if (s.pending != null) ...[
        PixelButton(label: '새로운 사무실 이야기!', primary: false, onPressed: _event),
        const SizedBox(height: 14),
      ],
      _goal(),
      const SizedBox(height: 17),
      const Text('오늘의 작은 업그레이드', style: TextStyle(fontSize: 18)),
      const SizedBox(height: 12),
      for (final u in content.upgrades) _upgrade(u),
  ...
  Widget _goal() {
    final requirements = engine.requirements(s);
    final met = requirements.where((r) => r.met).length;
    return PixelPanel(
      color: const Color(0xffe3ebd3),
      padding: const EdgeInsets.all(12),
      child: Row(
        children: [
          const PixelIcon('career', size: 25, color: green),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  s.rank < 4
                      ? '다음 목표: ${content.ranks[s.rank + 1]['name']}'
                      : '최고 직급 달성!',
                  style: const TextStyle(fontSize: 16),
                ),
                const SizedBox(height: 6),
                Text(
                  s.rank < 4 ? '성장 조건 $met / 5' : '나만의 커리어 완성!',
                  style: const TextStyle(fontSize: 12, color: muted),
                ),
              ],
            ),
